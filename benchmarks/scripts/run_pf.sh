...
if [ "$1" == "imagenet" ]; then
    ...
    if [ "$2" == "restart" ]; then
        ...
        resample_inds=31,35,39,43,47,51
    else
...
# Generate samples
python3 generate.py --network ${net} --outdir=samples/pf/${1}_${2}_${3} \
    --sampler $2 --method pf --cond=${cond} --dg_weight_1st_order=0.0 \
...
# Select best image
python select_top_k.py --indir samples/pf/${1}_${2}_${3} --outdir samples/pf/${1}_${2}_${3}/best --best_of_n $3
